...
>>>>>>> 903f9b87122853ce6284a0e96660933e243c0ae3:src/MerchantOrderDetails.js
  ...
  cancelDispatch() {
    ...
    this.showLoader();
    fetch(
      `${SERVER_URL}/mobile/cancel_merchant_dispatch/${this.state.orderParam.order_number}`,
      {
        method: 'GET',
      },
    )
...
>>>>>>> 903f9b87122853ce6284a0e96660933e243c0ae3:src/MerchantOrderDetails.js
  ...
  displayRatingButton() {
    if (
      this.state.orderParam.status == 'Order delivered' &&
      ...
      return (
        <View style={{flexDirection: 'row', width: '90%', alignSelf: 'center'}}>
          <TouchableOpacity
            ...
            onPress={() =>
              Linking.openURL(
                `http://rickreen.com/mobile/retailer_order_details_pdf/${this.state.orderParam.id}`,
              )
            }>
            <LinearGradient
              start={{x: 0, y: 0}}
  ...
  displayButton() {
    if (
      ...
      this.state.orderParam.status == 'Rider confirmed'
    ) {
      return (
        <View style={{flexDirection: 'row', width: '90%', alignSelf: 'center'}}>
          <TouchableOpacity
    ...
    return (
      <View style={styles.body}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity
            onPress={() => this.props.navigation.navigate('MerchantCart')}
            ...
            <View style={styles.itemView}>
              ...
              <View style={styles.item2}>
                <Text style={styles.txt10}>
                  {this.state.orderParam &&
                    this.state.orderParam.contact_person}
                </Text>
                <Text style={styles.txt20}>
                  {this.state.orderParam && this.state.orderParam.contact_phone}
                </Text>
              </View>
            </View>
            <View style={styles.itemView}>
              <View style={styles.item3}>
                ...
                <Text style={styles.txt}>
                  {this.state.orderParam && this.state.orderParam.address}
                </Text>
              </View>
            </View>
                ...
                this.state.orderDetails.map((orderDetail, index) => (
                  <View style={styles.item11}>
                    <Text style={styles.label40}>
                      {orderDetail.merchant_product_name} X{' '}
                      {orderDetail.order_detail_quantity}
                    </Text>
                    <Text style={styles.label}>
                      {orderDetail.merchant_product_description.substring(
                        0,
                        50,
                      )}
                    </Text>
                    <Text style={styles.label50}>
                      ₦
              ...
              <View style={styles.item3}>
                ...
                <View style={styles.item22}>
                  ...
                  <Text style={styles.labelZ}>
                    ₦
                    {this.state.orderParam &&
                      parseFloat(this.state.orderParam.total)
                        .toFixed(2)
                        .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                  </Text>
                </View>
                ...
                <View style={styles.item22}>
                  ...
                  <Text style={styles.labelZZ}>
                    ₦
                    {this.state.orderParam &&
                      parseFloat(this.state.orderParam.grand_total)
                        .toFixed(2)
                        .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                  </Text>
                </View>
