...
export class DispatchAddress extends Component {
    ...
    fetch(`${SERVER_URL}/mobile/get_vehicle_types`, {
      ...
      .then(res => {
        ...
        if (res.success) {
          this.setState(
            {
              vehicleTypes: res.vehicle_types.filter(function (vehicleType) {
  ...
  async submit() {
    ...
    if (this.state.pickupLongitude == '') {
      this.showAlert(
        'Error',
        'Kindly select a pickup address from sugestions provided',
      );
      return;
    ...
    if (this.state.deliveryLongitude == '') {
      this.showAlert(
        'Error',
        'Kindly select delivery address from sugestions provided',
      );
      return;
  ...
  getDistance(origin, destination, dispatchCart) {
    ...
    fetch(
      ...
      .catch(error => {
        ...
        Alert.alert(
          'Oops!',
          'Please correct address details',
          [
            {
              text: 'Ok',
              onPress: () => console.log('Cancel Pressed'),
  ...
  changeStatus = status => {
    if (this.state.customer.city_id == null) {
      this.showAlert(
        'Info',
        'Kindly register a new account to use this feature',
      );
      return;
    }
    var label = this.state.cities.find(
      x => x.id == this.state.customer.city_id,
    );
  ...
  displayPickeritems(vehicleType) {
    return (
      <Picker.Item
        color="#444"
        label={
          vehicleType.name + ' - Max weight: ' + vehicleType.max_weight + 'kg'
        }
        key={vehicleType.id}
    ...
    <View style={styles.bottomView}>
      ...
      <View style={styles.mySwitchView}>
        ...
        <CheckBox
          ...
          onValueChange={() => {
            this.changeStatus(!this.state.useAddress);
          }}
        />
      </View>
